...
import { deleteCommitteeAction } from "@/actions/committee.actions";

export function DeleteCommitteeButton({ id }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success("Success", { description: res.message || "Committee deleted successfully" });
      } else {
        toast.error("Error", { 
          description: res.errors?.length 
            ? `${res.message}\nDetails: ${res.errors.join(", ")}` 
            : res.message || "Failed to delete committee" 
        });
      }
    } catch (err) {
