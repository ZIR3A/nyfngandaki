...
import AboutActivity from '@/models/AboutActivity';
...
export async function GET(request) {
  try {
    ...
    const data = await AboutActivity.find({ provinceId, deletedAt: null }).sort({ displayOrder: 1 }).lean();
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item._id && existingIds.includes(item._id)) {
        await aboutService.updateActivity(item._id, payload, userId);
      } else {
        await aboutService.createActivity(provinceId, payload, userId);
      }
    }
