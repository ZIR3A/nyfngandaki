...
export async function GET(request) {
  try {
    ...
    const data = await AboutTimeline.find({ provinceId, deletedAt: null }).sort({ displayOrder: 1 }).lean();
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item._id && existingIds.includes(item._id)) {
        await aboutService.updateTimeline(item._id, payload, userId);
      } else {
        await aboutService.createTimeline(provinceId, payload, userId);
      }
    }
