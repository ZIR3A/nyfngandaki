...
export async function GET(request) {
  try {
    ...
    const data = await AboutPartner.find({ provinceId, deletedAt: null }).sort({ displayOrder: 1 }).lean();
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item._id && existingIds.includes(item._id)) {
        await aboutService.updatePartner(item._id, payload, userId);
      } else {
        await aboutService.createPartner(provinceId, payload, userId);
      }
    }
    ...
    console.error('Partners Sync error:', error);
    return NextResponse.json({ success: false, message: error.message }, { status: 500 });
  }
}
